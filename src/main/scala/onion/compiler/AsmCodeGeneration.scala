...
class AsmCodeGeneration(config: CompilerConfig) extends BytecodeGenerator:
  ...
  override def process(classes: Seq[TypedAST.ClassDefinition]): Seq[CompiledClass] =
    classes.map(generateClass)

  private def generateClass(classDef: ClassDefinition): CompiledClass =
    val cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES | ClassWriter.COMPUTE_MAXS)
    
    // Generate class header
    val access = toAsmModifier(classDef.modifier) | Opcodes.ACC_SUPER
    val name = classDef.name.replace('.', '/')
    val superName = if classDef.superClass != null then classDef.superClass.name.replace('.', '/') else "java/lang/Object"
    ...
    // Return compiled class with output path
    val outputPath = name + ".class"
  ...
  private def codeConstructor(cw: ClassWriter, ctor: ConstructorDefinition, className: String): Unit =
    ...
    val gen = new GeneratorAdapter(mv, toAsmModifier(ctor.modifier), "<init>", desc)
  ...
  private def emitExpressionWithContext(gen: GeneratorAdapter, expr: Term, className: String, localVars: LocalVarContext): Unit = expr match
    ...
    case ref: RefLocal =>
      if ref.frame == 0 then
        ...
        // Handle closure variables - would need outer frame access
        throw new UnsupportedOperationException("Closure variables not yet implemented")
    ...
    case set: SetField =>
      ...
      emitExpressionWithContext(gen, set.value, className, localVars)
      // Need to handle the order: target, value for putfield, but we want value as result
      val valueType = asmType(set.value.`type`)
      if valueType.getSize() == 2 then
        // Long or double - use dup2_x1
        gen.dup2X1()
        gen.pop2()
      else
        // Single slot value - use dup_x1  
        gen.dupX1()
        gen.pop()
  ...
  private def generateClosure(gen: GeneratorAdapter, closure: NewClosure, currentClassName: String, localVars: LocalVarContext): Unit =
    ...
    val capturedVars = if closure.frame != null then closure.frame.entries else Seq.empty
    ...
    // TODO: We need to store this closure class somewhere so it can be loaded
    // For now, we'll assume it's available at runtime
  ...
  private def generateClosureMethod(
    ...
    // Set up local variable context for the closure method
    val localVars = new LocalVarContext(gen).withParameters(false, method.arguments)
    
    // Map captured variables to field access
    val closureLocalVars = new LocalVarContext(gen) {
      override def getOrAllocateSlot(typedIndex: Int, tp: AsmType): Int =
        ...
        capturedVars.find(_.index == typedIndex) match
          case Some(capturedVar) =>
            // Load from field instead of local variable
            gen.loadThis()
            gen.getField(
              AsmType.getObjectType(className),
              s"captured_${capturedVar.index}",
            ...
            // Return a dummy slot since we've already pushed the value
            -1 // This will cause issues, need better approach
          case None =>
    ...
    emitStatementWithContext(gen, block, className, localVars)
